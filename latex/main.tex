...
% author names 
\author{Tudor Oancea, Yuning Jiang}% <-this % stops a space
...
\begin{abstract}
    ...
    % In this paper, we investigate the use of relaxed recentered logarithmic barrier functions in the context of nonlinear model predictive control.
...
Such modifications lead to equality-only-constrained or even unconstrained optimization problems that could be in some sense "easier to solve", but that still present some flaws, like the non-definition of these barrier functions outside the constraint set, which can lead to infeasible problems.
This is usually solved by introducing soft constraints, however no theoretical guarantees can be given on the convergence of the resulting optimization problem.
Another solution consists in \textit{relaxing} the barrier function by extending it outside the constraint set in a twice continuously differentiable fashion.
These \textit{relaxed (recentered) logarithmic barrier functions} (RRLB functions) have been introduced in [] and heavily studied in the context of stabilization with Linear MPC in \cite{RRLB} and \cite{RRLB-linear-MPC}.
In this simple case, it was proven that globally asymptotically stabilizing schemes can be desgined with the help of these functions, in particular by leveraging appropriate terminal costs and/or sets.
What's more, constraint satisfaction guarantees can be derived locally around the reference state.

The goal of this paper is to extend part of this results in the more general case of nonlinear dynamics.
In particular, we are going to use the fourth scheme described in \cite{RRLB-linear-MPC}, that only uses quadratic terminal costs and no terminal constraints, show that we can ensure asymptotic stability but only in a neighborhood of the reference state (Section \ref{sec:RRLB-nominal-stability}), and show that constraint satisfaction guarantees ... (Section \ref{sec:constraints-satisfaction-guarantees}).
We also aim at providing a numerical illustration of the theoretical results by using the RTI method and actually comparing a regular MPC scheme with its relaxed barrier function based counterpart (Section \ref{sec:numerical-experiments}).

In this paper we will call $\calC^k$ the set of all $k$-times continuously differentiable functions, and we will call $D_xf(x,u)$ and $\nabla^2_xf(x,u)$ the jacobian and the hessian with respect to $x$ of a function $f$ defined on $R^{n_x}\times\R^{n_u}$.
